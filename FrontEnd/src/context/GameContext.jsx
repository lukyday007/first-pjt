...
export const GameProvider = ({ children }) => {
  const [gameRoomId, setGameRoomId] = useState(1); // 게임 방 번호
  ...
  const [myLocation, setMyLocation] = useState({ lat: 0, lng: 0 }); // 내 위치 정보, { lat: 0.0, lng: 0.0 }
  ...
  useEffect(() => {
    ...
    if (myLocation && areaCenter) {
      // console.log(`myLocation: ${myLocation.lat} ${myLocation.lng}`);
      // console.log(`areaCenter: ${areaCenter.lat} ${areaCenter.lng}`);
      setDistance(
        harversineDistance(
          myLocation.lat,
          myLocation.lng,
