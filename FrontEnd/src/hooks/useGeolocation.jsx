...
//   const [distance, setDistance] = useState(0);
//   // 거리 계산 시간
//   const [distanceCalcTime, setDistanceCalcTime] = useState(null);
...
//       setDistanceCalcTime((end - start).toFixed(2));
//     }
//   }, [location, areaCenter]);
...
//   // 화면에 거리 계산 시간을 표시하는 부분
//   useEffect(() => {
//     if (distanceCalcTime) {
//       const resultElem = document.getElementById('distance-calc-time');
//       if (resultElem) {
//         resultElem.innerText = `거리 계산 시간: ${distanceCalcTime} ms`;
//       }
//     }
//   }, [distanceCalcTime]);

//   return { isLoading, location, areaCenter, distance, distanceCalcTime };
// };
...
const useGeolocation = () => {
  ...
  const updateDistance = useCallback(() => {
    if (location && areaCenter) {
      const calculateDistanceAsync = async () => {
        const start = performance.now();
        const calculatedDistance = await new Promise(resolve => {
          setTimeout(() => {
            resolve(
              approximateDistance(
                location.lat,
        ...
        setDistanceCalcTime((end - start).toFixed(5));
      };
  ...
  useEffect(() => {
    if (distanceCalcTime) {
      const resultElem = document.getElementById('distance-calc-time');
      if (resultElem) {
        resultElem.innerText = `거리 계산 시간: ${distanceCalcTime} 밀리초`;
      }
    }
  }, [distanceCalcTime]);
