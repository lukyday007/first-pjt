    ...
    ws.onmessage = async event => {
      ...
      if (toUser === participantName) {
        ...
        if (signal.type === "offer") {
          ...
          ws.send(
            `click:${JSON.stringify(answer)}:${fromUser}:${participantName}`
          );
    ...
    peerConnection.current.onicecandidate = event => {
      ...
      if (
        ...
        webSocket.current.send(
          `click:${JSON.stringify({ type: "ice-candidate", candidate: event.candidate })}:${recipient}:${participantName}`
        );
      }
    };
          ...
          <div className="mt-4">
            {filteredRemoteTracks.map(remoteTrack => (
              <Button
                ...
                onClick={() =>
                  handleButtonClick(remoteTrack.participantIdentity)
                }
                variant="default"
              >
